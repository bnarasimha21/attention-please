...
export const Scene7WhyMatters: React.FC = () => {
  ...
  const targetSpring = pop(frame, fps, fps * 3.2, { damping: 10 });
  ...
  const chip1Pop = pop(frame, fps, fps * 4.8, { damping: 11 });
  const chip2Pop = pop(frame, fps, fps * 6.1, { damping: 11 });
  ...
  return (
    <AbsoluteFill>
      ...
      <div style={{ position: "absolute", bottom: 200, width: "100%", display: "flex", justifyContent: "center", gap: 48 }}>
        <div style={{
          opacity: chip1, transform: `translateY(${(1 - chip1) * 16}px) scale(${interpolate(chip1Pop, [0, 1], [0.85, 1])})`,
        ...
        <div style={{
          opacity: chip2, transform: `translateY(${(1 - chip2) * 16}px) scale(${interpolate(chip2Pop, [0, 1], [0.85, 1])})`,
          padding: "17px 31px", borderRadius: 14,
